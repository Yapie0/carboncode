...
describe("isNpxInstall", () => {
  ...
  it("detects _npx path fragment", () => {
    process.argv[1] = "/Users/x/.npm/_npx/abc123/node_modules/.bin/carboncode";
  ...
  it("detects npx via user-agent string", () => {
    process.argv[1] = "/usr/local/bin/carboncode";
    process.env.npm_config_user_agent = "npx/10.2.4 npm/10.2.4 node/v20.10.0";
    expect(isNpxInstall()).toBe(true);
  });

  it("returns false for plain global install", () => {
    process.argv[1] = "/usr/local/lib/node_modules/@carboncode/cli/dist/cli/index.js";
    // biome-ignore lint/performance/noDelete: cover the no-env case
    delete process.env.npm_config_user_agent;
...
describe("detectInstallSource", () => {
  ...
  it("identifies npm via Windows %APPDATA%/npm path", () => {
    expect(
      detectInstallSource(
        "C:\\Users\\me\\AppData\\Roaming\\npm\\node_modules\\@carboncode\\cli\\dist\\cli\\index.js",
      ),
    ).toBe("npm");
  });
  ...
  it("identifies npm via nvm path", () => {
    expect(
      detectInstallSource(
        "/Users/me/.nvm/versions/node/v22.11.0/lib/node_modules/@carboncode/cli/dist/cli/index.js",
      ),
    ).toBe("npm");
  });
  ...
  it("identifies bun via .bun install dir", () => {
    expect(
      detectInstallSource(
        "/Users/me/.bun/install/global/node_modules/@carboncode/cli/dist/cli/index.js",
      ),
    ).toBe("bun");
  });
  ...
  it("identifies pnpm via pnpm/global", () => {
    expect(
      detectInstallSource(
        "/Users/me/.local/share/pnpm/global/5/node_modules/@carboncode/cli/dist/cli/index.js",
      ),
    ).toBe("pnpm");
  });
  ...
  it("identifies yarn via yarn/global", () => {
    expect(
      detectInstallSource(
        "/Users/me/.config/yarn/global/node_modules/@carboncode/cli/dist/cli/index.js",
      ),
    ).toBe("yarn");
  });

  it("identifies npx via _npx fragment", () => {
    expect(detectInstallSource("/Users/me/.npm/_npx/abc/node_modules/.bin/carboncode")).toBe("npx");
  });

  it("returns unknown for paths that match no known pattern", () => {
    expect(detectInstallSource("/opt/custom/bin/carboncode")).toBe("unknown");
  });
...
describe("detectNpmInstallPrefix", () => {
  it("extracts the prefix from a POSIX lib/node_modules path", () => {
    expect(
      detectNpmInstallPrefix("/usr/local/lib/node_modules/@carboncode/cli/dist/cli/index.js"),
    ).toBe("/usr/local");
  });
...
describe("getLatestVersion", () => {
  ...
  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), "carboncode-version-"));
  });
  ...
  it("fetches from the registry and caches the result", async () => {
    ...
    const cacheFile = join(home, ".carboncode", "version-cache.json");
  ...
  it("force:true bypasses the cache", async () => {
    writeFileSync(join(home, ".carboncode-cache-preseed.json"), ""); // just ensures the tmp dir is real
    // Preseed the cache directly.
    const { mkdirSync } = await import("node:fs");
    mkdirSync(join(home, ".carboncode"), { recursive: true });
    writeFileSync(
      join(home, ".carboncode", "version-cache.json"),
