...
describe("handleSlash", () => {
  ...
  it("/undo outside code mode says it's not available", () => {
    ...
    expect(r.info).toMatch(/only available inside `carboncode code`/);
  });
  ...
  it("/restore outside code mode is unavailable regardless of args", () => {
    ...
    expect(noArg.info).toMatch(/only available inside `carboncode code`/);
    const withArg = handleSlash("restore", ["abc"], makeLoop());
    expect(withArg.info).toMatch(/only available inside `carboncode code`/);
  });
  ...
  it("suggestSlashCommands filters by prefix", () => {
    ...
    expect(suggestSlashCommands("", true)).toHaveLength(59);
    ...
    expect(suggestSlashCommands("", true).map((s) => s.cmd)).toContain("multi-agent");
  ...
  describe("/update", () => {
    it("reports pending check when latestVersion is null (offline / in flight)", () => {
      ...
      expect(r.info).toMatch(/current: carboncode/);
      expect(r.info).toMatch(/not yet resolved/);
      expect(r.info).toMatch(/carboncode update/);
    });
    ...
    it("prints shell command when latest is newer than current", () => {
      ...
      expect(r.info).toMatch(/carboncode update/);
      expect(r.info).toMatch(/npm install -g @carboncode\/cli@latest/);
    });
  ...
  describe("/mcp disable / enable", () => {
    ...
    beforeEach(() => {
      tempHome = mkdtempSync(join(tmpdir(), "carboncode-mcp-toggle-"));
    ...
    it("/mcp disable <name> persists the name into config.mcpDisabled", () => {
      ...
      const cfgPath = join(tempHome, ".carboncode", "config.json");
    ...
    it("/mcp enable <name> removes from disabled and clears the array when empty", () => {
      const cfgPath = join(tempHome, ".carboncode", "config.json");
      mkdirSync(join(tempHome, ".carboncode"), { recursive: true });
  ...
  describe("/memory", () => {
    ...
    it("prints a how-to when no project or user memory exists", () => {
      const r = handleSlash("memory", [], makeLoop(), { memoryRoot: root });
      expect(r.info).toMatch(/no memory pinned/);
      expect(r.info).toMatch(/CARBON\.md/);
    });

    it("prints the CARBON.md contents + path when present", () => {
      writeFileSync(
        join(root, "CARBON.md"),
    ...
    it("says memory is disabled when CARBONCODE_MEMORY=off, even with a file present", () => {
      writeFileSync(join(root, "CARBON.md"), "content", "utf8");
