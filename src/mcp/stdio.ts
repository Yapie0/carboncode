  ...
  private onStdout(chunk: string): void {
    ...
    while ((newlineIdx = this.stdoutBuffer.indexOf("\n")) !== -1) {
      ...
      try {
        ...
        // banners before the JSON-RPC loop begins. Surface only when MCP
        // stdio debug is enabled; otherwise the noise corrupts the TUI render.
        if (mcpStdioDebugEnabled()) {
...
export function mcpStdioDebugEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return (env.CARBONCODE_DEBUG_MCP ?? env.REASONIX_DEBUG_MCP) === "1";
}
