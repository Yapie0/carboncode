...
export interface SlashContext {
  ...
  memoryRoot?: string;
  /** Override the Carbon home lookup root — production leaves this absent (defaults to `os.homedir()`); tests inject a tmpdir so they don't read the dev's real global memory. */
  homeDir?: string;
  planMode?: boolean;
  editMode?: EditMode;
  ...
  /** `/add-dir <path>` — register an extra allowed root for file + shell tools (bare = list current roots). Code mode only. */
  addDir?: (dir: string) => { info: string };
