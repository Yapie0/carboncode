...
export const zhCN: TranslationSchema = {
  ...
  slash: {
    ...
    pricing: {
      description: "显示成本计算使用的模型价格、配置覆盖示例和官方参考页面",
      argsHint: "[open]",
    },
  ...
  wizard: {
    ...
    apiKeyOpenedBrowser: "已为你打开浏览器获取 key（没弹出？访问下方网址，或按 Tab 重新打开）。",
    apiKeyOpenHint: "按 Tab 打开浏览器获取 key。",
    apiKeyShownOnce: "DeepSeek 只显示一次，记得马上复制。",
    ...
    apiKeyFooter: "[Enter] 确认 · [Tab] 打开 key 页面 · [Ctrl+B] 返回 · [Esc] 取消",
    themeTitle: "选择主题",
    themeSubtitle: "方向键切换时即时预览效果，之后可用 /theme 更改。",
    themeSampleHeading: "示例",
    themeFooter: "[↑↓] 移动 · [Enter] 确认 · [Backspace/Ctrl+B] 返回 · [Esc] 取消",
    themeCaption: {
    ...
    mcpFooterMulti:
      "[↑↓] 移动  ·  [空格] 选择  ·  [Enter] 确认  ·  [Backspace/Ctrl+B] 返回  ·  [Esc] 取消  ·  全不选 = 跳过",
    ...
    mcpArgsFooter: "[Enter] 确认 · [Ctrl+B] 返回 · [Esc] 取消",
    reviewTitle: "确认保存",
    ...
    reviewFooter: "[Enter] 保存 · [Backspace/Ctrl+B] 返回 · [Esc] 取消",
    savedTitle: "▸ 已保存。",
    savedFooter: "[Enter] 退出",
    selectFooter: "[↑↓] 移动 · [Enter] 确认 · [Esc] 取消",
    selectFooterBack: "[↑↓] 移动 · [Enter] 确认 · [Backspace/Ctrl+B] 返回 · [Esc] 取消",
  ...
  loop: {
    ...
    deepThinkOn: "⬢ 深度思考已开启 — 本轮使用 {model}（一次性）",
    ...
    continuingAfterOutputLimit:
      "\u5355\u6b21\u8f93\u51fa\u5df2\u8fbe\u670d\u52a1\u5546\u4e0a\u9650\uff0c\u6b63\u5728\u81ea\u52a8\u7eed\u5199\uff08{attempt}/{max}\uff09\u2026",
  ...
  handlers: {
    ...
    admin: {
      ...
      updateCurrent: "当前：carboncode {version}",
      updateLatestPending: "最新：（尚未解析 — 后台检查进行中或离线）",
      updateRetryHint: "已触发新的注册表获取 — 几秒后重试 `/update`，",
      updateRetryHint2: "或在另一个终端运行 `carboncode update` 强制同步执行。",
      updateLatest: "最新：carboncode {version}",
      updateUpToDate: "您已是最新版本。无需操作。",
      updateNpxHint: "您正在通过 npx 运行 — 下次 `npx @carboncode/cli ...` 启动时将自动获取。",
      updateNpxForce: "要强制刷新：`npm cache clean --force`。",
      updateUpgradeHint: "要升级，请退出此会话并运行：",
      updateUpgradeCmd1: "  carboncode update           （交互式，支持 --dry-run 预览）",
    ...
    edits: {
      undoCodeOnly: "/undo 仅在 `carboncode code` 中可用 — 聊天模式不应用编辑。",
      historyCodeOnly: "/history 仅在 `carboncode code` 中可用。",
      showCodeOnly: "/show 仅在 `carboncode code` 中可用。",
      applyCodeOnly: "/apply 仅在 `carboncode code` 中可用（此处无内容可应用）。",
      discardCodeOnly: "/discard 仅在 `carboncode code` 中可用。",
      planCodeOnly: "/plan 仅在 `carboncode code` 中可用 — 聊天模式不限制工具写入。",
      planOn:
      ...
      modeCodeOnly: "/mode 仅在 `carboncode code` 中可用。",
      ...
      commitCodeOnly: "/commit 仅在 `carboncode code` 中可用（需要有根的 git 仓库）。",
      commitUsage: '用法：/commit "提交消息"  — 在 {root} 中运行 `git add -A && git commit -m "…"`',
      walkCodeOnly: "/walk 仅在 `carboncode code` 中可用。",
      checkpointCodeOnly: "/checkpoint 仅在 `carboncode code` 中可用 — 聊天模式不应用编辑。",
      checkpointNone:
      ...
      restoreCodeOnly: "/restore 仅在 `carboncode code` 中可用。",
    ...
    permissions: {
      mutateCodeOnly:
        "/permissions add / remove / clear 仅在 `carboncode code` 中可用 — 它们编辑项目范围的允许列表（`~/.carboncode/config.json` projects[<root>].shellAllowed）。",
      addUsage:
    ...
    observability: {
      ...
      pricingTitle: "{model} 的价格",
      pricingCurrent:
        "  USD / 1M tokens：缓存命中输入 {hit}，缓存未命中输入 {miss}，输出 {output}（{source}）",
      pricingMissing: '  暂无 "{model}" 的价格表；成本将显示为 0。',
      pricingFormula:
        "  公式：(cacheHitTokens*hit + cacheMissTokens*miss + completionTokens*output) / 1,000,000",
      pricingReference: "  参考：{url}",
      pricingOverrideIntro: "  在 {path} 中覆盖：",
      pricingSourceBuiltIn: "内置价格表",
      pricingSourceOverride: "配置覆盖",
      pricingSourceMissing: "缺少价格表",
      pricingOpenHint: "  运行 /pricing open 打开参考页面。",
      pricingOpened: "  已在浏览器中打开参考页面。",
      pricingOpenFailed: "  无法打开参考页面：{reason}",
